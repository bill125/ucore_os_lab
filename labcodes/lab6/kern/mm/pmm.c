...
get_pte(pde_t *pgdir, uintptr_t la, bool create) {
    ...
    if ((*pdep & PTE_P) == 0) {                         // (2) check if entry is not present
        ...
        uintptr_t pa = KADDR(page2pa(page));            // (5) get linear address of page
        memset((void *)pa, 0, PGSIZE);                         // (6) clear page content using memset
        *pdep = page2pa(page) | PTE_P | PTE_W | PTE_U;     // (7) set page directory entry's permission
    }
...
copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end, bool share) {
    ...
    do {
         ...
         */
            const void *src_kvaddr = page2kva(page);
